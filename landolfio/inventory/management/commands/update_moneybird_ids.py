...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "json_file", type=str, help="Path to JSON file with asset moneybird IDs"
        )
    ...
    def _wait_for_rate_limit(self, error):
        ...
        if match:
            ...
            self.stdout.write(
                self.style.WARNING(f"Rate limited, waiting until {retry_after}...")
            )
        ...
        for asset_name, asset_data in data.items():
            ...
            try:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Updated {asset_name}: {moneybird_id}, synced name to Moneybird, and refreshed data"
                        )
                    )
                except Exception as e:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Updated {asset_name}: {moneybird_id} but failed to sync: {e}"
                        )
                    )

                updated_count += 1
            except Asset.DoesNotExist:
                self.stdout.write(self.style.ERROR(f"Asset not found: {asset_name}"))
                not_found_count += 1
